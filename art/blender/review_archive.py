"""Focused archive review and GLB round-trip audit, through Blender MCP."""
import bpy,json,os
...
report={}
for lod in [0,1]:
    scene=bpy.data.scenes.new('Archive_Inspection');bpy.context.window.scene=scene
    scene.world=reference.world
    scene.render.engine='CYCLES';scene.cycles.samples=24;scene.cycles.use_denoising=True
    scene.render.resolution_x=1100;scene.render.resolution_y=1100;scene.render.resolution_percentage=100
    scene.view_settings.view_transform='AgX'
    path=f'{ROOT}/art/blender/ENV_Civic_archive_LOD{lod}.glb'
    ...
    camd=bpy.data.cameras.new('Archive_Camera');cam=bpy.data.objects.new('Archive_Camera',camd);scene.collection.objects.link(cam);scene.camera=cam
    camd.type='ORTHO';camd.ortho_scale=12
    cam.location=(12,-24,12);cam.rotation_euler=(Vector((-.5,0,3.7))-cam.location).to_track_quat('-Z','Y').to_euler()
    scene.render.filepath=OUT+('/archive-front.png' if lod==0 else '/archive-mobile.png');bpy.ops.render.render(write_still=True)
...
with open(OUT+'/archive-technical-review.json','w') as f:json.dump(report,f,indent=2)
print('ARCHIVE_REVIEW',report)
