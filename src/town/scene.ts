...
const daylightSun=new THREE.Color(0xffedcf),overcastSun=new THREE.Color(0xdce5f0),nightSun=new THREE.Color(0x9bb6e2);
...
export class TownScene {
  ...
  private buildRoads(snapshot:TownSnapshot){
    ...
    const stone=(x:number,z:number,seed:number,width:number)=>{
      ...
      paving.push({family:'Rock_Path',x,y:terrainHeight(x,z)+.045,z,sx:size,sy:.065+(variation%4)*.008,sz:size*(.76+((variation>>>5)%19)/100),rotation:((variation>>>8)%628)/100});
    };
    ...
    for(const ring of lists){
      ...
      for(let i=0;i<ring.stone;i++){
        ...
        for(let row=0;row<(this.mobile?2:4);row++)for(let col=0;col<4;col++){
          ...
          for(const side of [-1,1])facing.push({family:'Rock_Path',x:x+side*x/distance*.4,y:.68+row*(this.mobile?1.65:.91),z:z+side*z/distance*.4,
  ...
  private updateAtmosphere(snapshot:TownSnapshot){
    ...
    this.sun.color.set(0xffdfad).lerp(daylightSun,daylight*.45)
      .lerp(overcastSun,overcast*.85).lerp(nightSun,night);
    this.sun.intensity=(3.05-overcast*1.8)*(1-.82*night);
    this.fill.color.set(0xb8c8eb).lerp(overcastFill,overcast*.6).lerp(nightFill,night*.7);
    this.fill.groundColor.set(0x9d806a).lerp(nightGround,night*.8);
    ...
    this.fogColor.set(0xe5ddcd).lerp(overcastFog,overcast).lerp(nightFog,night*.78);
